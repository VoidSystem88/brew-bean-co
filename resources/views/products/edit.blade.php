                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <div class="image-upload-container" onclick="document.getElementById('imageInput').click()">
                                    @if($product->image && file_exists(storage_path('app/public/products/' . $product->image)))
                                        <img id="imagePreview" src="{{ asset('storage/products/' . $product->image) }}" alt="{{ $product->name }}">
                                ...
                                @if($product->image && file_exists(storage_path('app/public/products/' . $product->image)))
                                    <div class="current-image">
                                        <i class="fas fa-check-circle text-success me-1"></i> Current image: {{ $product->image }}
                                    </div>
                                @endif
                                <small class="text-muted">Click the box to upload new image. Max 5MB. JPG, PNG, GIF supported.</small>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="category" id="category" class="form-select @error('category') is-invalid @enderror">
                                    ...
                                    <option value="Sandwich" {{ old('category', $product->category) == 'Sandwich' ? 'selected' : '' }}>🥪 Sandwich</option>
    ...
    function applyCrop() {
        ...
        canvas.toBlob(function(blob) {
            ...
            reader.onload = function(e) {
                ...
                showFeedback('Image cropped and ready to update!', 'success');
            };
            reader.readAsDataURL(file);
