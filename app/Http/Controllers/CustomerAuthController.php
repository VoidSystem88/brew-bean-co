...
class CustomerAuthController extends Controller
{
    ...
    private function getAvailableDiscounts($points, $subtotal = 0)
    {
        ...
        foreach ($discounts as $discount) {
            if ($points >= $discount['points'] && $subtotal >= $discount['min_purchase']) {
                $available[] = $discount;
            }
        }
        return $available;
    ...
    public function getAvailableDiscountsAjax(Request $request)
    {
    ...
    public function loyaltyPoints()
    {
        ...
        $pointsHistory = $customer->sales()
            ...
            ->select('id', 'total_amount', 'sale_date', 'discount_rate', 'discount_type', 'points_used', 'discount_amount')
        ...
        return view('customer.loyalty', compact(
            'customer', 'currentPoints', 'tiers', 'currentTier', 
            'nextTier', 'pointsHistory', 'availableDiscounts', 'allVouchers'
        ));
    }
    ...
    public function placeOrder(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'use_points_discount' => 'nullable|boolean',
            ...
            if ($activeVoucher && isset($activeVoucher['id'])) {
                if ($totalAmount >= $activeVoucher['min_purchase']) {
                    $voucherDiscount = $activeVoucher['value'];
                    $voucherLabel = $activeVoucher['label'];
                    session()->forget('active_voucher');
                }
            }
            ...
            // Points discount
            $usePointsDiscount = $request->boolean('use_points_discount');
            $selectedDiscountId = $request->discount_id ?? null;
            $discountValue = 0;
            $discountLabel = '';
            $pointsUsed = 0;
            $discountType = null;

            if ($usePointsDiscount && $selectedDiscountId && $customer) {
                $availableDiscounts = $this->getAvailableDiscounts($customer->loyalty_points, $totalAmount);
                $selected = collect($availableDiscounts)->firstWhere('id', $selectedDiscountId);
                
                if ($selected && $customer->loyalty_points >= $selected['points']) {
                    if ($totalAmount >= $selected['min_purchase']) {
            ...
            $sale = Sale::create([
                ...
                'discount_amount' => $discountValue + $voucherDiscount,
                'discount_rate' => 0,
            ...
            return response()->json([
                ...
                'order_id' => $sale->id,
                'total' => $finalTotal,
                'original_total' => $totalAmount + $voucherDiscount,
